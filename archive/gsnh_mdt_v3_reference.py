...
class LanguageFamily(Enum):
    ...
    BEST_PER_NODE = "BestPerNode"  # Journal-safe: pick best UI family per node
...
class CompareLiteral:
    ...
    x_i ≤ x_j is equivalent to ¬(x_i ≥ a) ∨ (x_j ≥ a) for all relevant a,
    ...
    def negate(self) -> 'CompareLiteral':
        """Negation: ¬(x≤y) ≡ x>y, ¬(x<y) ≡ x≥y, ¬(x≥y) ≡ x<y, ¬(x>y) ≡ x≤y"""
        neg_map = {
...
def select_language_via_cv(X: np.ndarray, y: np.ndarray,
                           candidates: Optional[List[LanguageFamily]] = None,
                           depth: int = 6,
                           min_samples_leaf: int = 5,
                           n_folds: int = 3,
                           lambda_size: float = 0.01,
                           lambda_expl: float = 0.0,
                           random_state: int = 42) -> LanguageFamily:
    """
    Select best language via cross-validation WITHOUT violating journal constraints.
    
    Trains one model per candidate language with mode='journal' and fixed language,
    then picks the best according to:
        score = accuracy - lambda_size * n_nodes - lambda_expl * avg_explanation_size
    
    Returns the selected language. The final tree must be trained with this
    fixed language and mode='journal'.
    """
    if candidates is None:
        candidates = [LanguageFamily.HORN, LanguageFamily.ANTI_HORN, 
                      LanguageFamily.AFFINE, LanguageFamily.SQUARE_2CNF]
    
    candidates = [c for c in candidates if c != LanguageFamily.ANY]
    ...
    for lang in candidates:
        ...
        for fold in range(n_folds):
            ...
            try:
                tree = ExpertGSNHTree(
                    stopping_criteria=StoppingCriteria(max_depth=depth, 
                ...
                size_penalty = lambda_size * tree.n_nodes_
                ...
                if lambda_expl > 0:
                    ...
                    for idx in sample_idx:
                        axp = tree.extract_axp(X_test[idx])
                        total_expl_size += len(axp)
                    expl_cost = lambda_expl * (total_expl_size / len(sample_idx))
                
                score = acc - size_penalty - expl_cost
                fold_scores.append(score)
                
            except Exception as e:
                print(f"Warning: Language {lang.value} failed in fold {fold}: {e}")
...
class GSNHPredicate:
    ...
    For Horn: a Horn clause (≤1 positive literal)
    For Square2CNF: a conjunction of 2-clauses (but we represent as single split)
    """
    literals: Tuple[Literal, ...]
    information_gain: float = 0.0
    language_family: LanguageFamily = LanguageFamily.HORN
    is_xor: bool = False
    is_compare: bool = False  # True if uses CompareLiteral
    ...
    def __post_init__(self):
        ...
        # Check for CompareLiteral or GSNHBinaryLiteral
        has_compare = any(isinstance(l, (CompareLiteral, GSNHBinaryLiteral)) for l in self.literals)
        ...
        if self.language_family == LanguageFamily.HORN:
            if n_positive > 1:
                raise ValueError(f"Horn violation: {n_positive} positive literals. Max allowed: 1. Literals: {[str(l) for l in self.literals]}")
        elif self.language_family == LanguageFamily.ANTI_HORN:
            if n_negative > 1:
                raise ValueError(f"Anti-Horn violation: {n_negative} negative literals. Max allowed: 1. Literals: {[str(l) for l in self.literals]}")
        # Square2CNF: checked at formula level, single split can be any 2-literal
        # Affine: any polarity combo is fine
    ...
    def _classify(self) -> GSNHPatternType:
        # Check for compare literals first
        if self.is_compare:
    ...
    def evaluate(self, X: np.ndarray) -> np.ndarray:
        """Evaluate predicate: True if the constraint is satisfied."""
        if self.is_xor:
            # Affine: XOR of all literals
            result = self.literals[0].evaluate(X)
            for lit in self.literals[1:]:
                result = result ^ lit.evaluate(X)
            return result
        else:
            # Horn/AntiHorn/2CNF: disjunction of literals (OR semantics for split)
            # This defines the "positive" branch of the split
            result = self.literals[0].evaluate(X)
            for lit in self.literals[1:]:
                result = result | lit.evaluate(X)
            return result
            ...
            for lit in self.literals:
                ...
                if feat not in S:
                    if isinstance(lit, (CompareLiteral, GSNHBinaryLiteral)):
                        ...
                        return None
            # All features assigned, can evaluate
            return bool(self.evaluate(x.reshape(1, -1))[0])
        else:
            # For OR: if any literal is True, whole thing is True
            # If all literals are False, whole thing is False
            # If some unknown and none True, result is unknown
            all_false = True
            for lit in self.literals:
                if isinstance(lit, (CompareLiteral, GSNHBinaryLiteral)):
                    feat_i, feat_j = lit.feature_i, lit.feature_j
                    if feat_i not in S or feat_j not in S:
                        all_false = False  # Can't confirm all false
                        continue
...
# =============================================================================
# EXACT SAT SOLVERS FOR TRACTABLE LANGUAGES
# =============================================================================

class ExactSATSolver:
    """
    Exact SAT solvers for each tractable language L.
    These are used for weak-AXp checking in journal mode.
    ...
    def horn_sat(clauses: List[List[Tuple[int, bool]]]) -> bool:
        ...
        A Horn clause is: (¬a₁ ∨ ... ∨ ¬aₖ ∨ b) ≡ (a₁ ∧ ... ∧ aₖ → b)
        Represented as list of (var, is_positive) where at most one is positive.
        
        Special case: all-negative clause (¬a₁ ∨ ... ∨ ¬aₖ) is unsatisfiable 
        if all aᵢ are true.
        """
        # Extract implications and all-negative clauses
        implications = []  # (body_vars, head_var or None)
        all_negative = []  # sets of vars that cannot all be true
        ...
        for clause in clauses:
            ...
            if len(pos_lits) == 0:
                # All-negative clause: body must not all be satisfied
                all_negative.append(set(v for v, _ in neg_lits))
            elif len(pos_lits) == 1:
                # Horn implication: neg_lits → pos_lit
                head = pos_lits[0][0]
        ...
        # Forward chaining: start with empty true set, derive consequences
        true_vars = set()
        changed = True
    ...
    def antihorn_sat(clauses: List[List[Tuple[int, bool]]]) -> bool:
        ...
        Anti-Horn: ≤1 negative literal per clause.
    ...
    def two_sat(clauses: List[List[Tuple[int, bool]]]) -> bool:
        """
        Exact 2-SAT via implication graph and SCC.
        
        Each clause (a ∨ b) becomes (¬a → b) ∧ (¬b → a).
        ...
        # Build implication graph: 2*i = false, 2*i+1 = true
        # Literal (v, s) maps to node 2*idx[v] + (1 if s else 0)
        def lit_node(v, s):
    ...
    @staticmethod
    def affine_sat(equations: List[Tuple[Set[int], int]]) -> bool:
        """
        Exact Affine-SAT via Gaussian elimination over GF(2).
        
        Each equation is (vars, const) representing: ⊕_{v in vars} v = const (mod 2)
        """
        if not equations:
            return True
        ...
        if not all_vars:
            # Check if any equation has const=1
...
def brute_force_sat_verify(clauses: List[List[Tuple[int, bool]]], n_vars: int) -> bool:
    ...
    for i in range(1 << n_vars):
        assignment = [(i >> v) & 1 for v in range(n_vars)]
        sat = True
        for clause in clauses:
            clause_sat = False
            for v, is_pos in clause:
                val = assignment[v]
                if (val and is_pos) or (not val and not is_pos):
                    clause_sat = True
                    break
            if not clause_sat:
                sat = False
...
class ExpertGSNHTree:
        ...
        if self.mode == 'journal':
            if self.language == LanguageFamily.ANY:
                raise ValueError(
                    ...
                    "language=HORN/ANTI_HORN/AFFINE/SQUARE_2CNF/BEST_PER_NODE explicitly."
                )
    ...
    def _search_best_split(self, X, y, feature_scores,
        ...
        search_horn = language in (LanguageFamily.ANY, LanguageFamily.HORN, LanguageFamily.SQUARE_2CNF, LanguageFamily.BEST_PER_NODE)
        search_antihorn = language in (LanguageFamily.ANY, LanguageFamily.ANTI_HORN, LanguageFamily.SQUARE_2CNF, LanguageFamily.BEST_PER_NODE)
    ...
    def _is_sat_path(self, path_edges, x, S):
        ...
        if self.language == LanguageFamily.AFFINE:
            return self._affine_path_sat(path_edges, x, S)
        # All UI families (Horn, Anti-Horn, Square2CNF) use numeric interval SAT
        return self._path_sat_numeric(path_edges, x, S)
    ...
    def _path_sat_numeric(self, path_edges, x, S) -> bool:
        ...
        for pred, branch in path_edges:
            ...
            if branch:
                # Predicate is TRUE → OR of literals is true → at least one literal satisfied
                # Collect as a disjunction clause to be resolved later
                clause_lits = []
                for lit in pred.literals:
                    if isinstance(lit, (CompareLiteral, GSNHBinaryLiteral)):
                        continue  # Skip relational literals in interval SAT
                    clause_lits.append(lit)
                if clause_lits:
                    or_clauses.append(clause_lits)
            else:
                # Predicate is FALSE → ALL literals are false → conjunction of negations
                for lit in pred.literals:
                    if isinstance(lit, (CompareLiteral, GSNHBinaryLiteral)):
                        continue
                    # Negate the literal and intersect its interval
                    neg_lit = lit.negate()
                    lo_new, hi_new = neg_lit.to_interval()
                    f = neg_lit.feature
        ...
        # Now solve the OR-clauses via DFS case-splitting
        return self._solve_or_clauses_dfs(or_clauses, intervals, x, S, 0)
    ...
    def _solve_or_clauses_dfs(self, or_clauses, intervals, x, S, idx) -> bool:
        ...
        for lit in clause:
            ...
            if self._solve_or_clauses_dfs(or_clauses, intervals_copy, x, S, idx + 1):
                return True
    ...
    def _affine_path_sat(self, path_edges, x, S) -> bool:
        ...
        """
        equations = []
        # Fixed features: for each f in S, determine the boolean value
        # using the literal's threshold context. Since affine predicates
        # use LT-polarity literals, the boolean is: b_f = 1 iff x[f] < threshold.
        # But we need a consistent booleanization per feature.
        # Convention: b_f = 1 iff x[f] >= threshold (GE polarity).
        # We collect threshold-specific equations from each XOR predicate.

        # Collect all thresholds used per feature across XOR predicates
        feature_thresholds = {}
        for pred, branch in path_edges:
            if not pred.is_xor:
                continue
            for lit in pred.literals:
                if isinstance(lit, (CompareLiteral, GSNHBinaryLiteral)):
                    continue
                f = lit.feature
                if f not in feature_thresholds:
                    feature_thresholds[f] = lit.threshold

        # Unit constraints for fixed features
        for f in S:
            if f in feature_thresholds:
                thr = feature_thresholds[f]
                val = 1 if x[f] >= thr else 0
                equations.append((set([f]), val))
