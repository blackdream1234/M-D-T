...
## Mixed affine+numeric empirical paths

Affine paths in the Python implementation remain auxiliary until a verified GF(2) certificate checker is implemented. Mixed affine+numeric paths are therefore excluded from theorem-certified benchmark rows. The existing path checks preserve soundness for empirical explanation validation, but subset-minimality can be over-conservative because the deletion-based AXp extractor does not solve a combined numeric-interval plus GF(2) minimality problem. This does not affect theorem-certified rows, which exclude Affine and fallback backends.

BestPN remains empirical: it chooses the locally best split among the families actually searched at a node, does not include Square2CNF, and excludes Affine by default unless `allow_affine_in_bestpn=True`.
