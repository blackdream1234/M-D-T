...
def import_gsnh():
    """Import GSNH module — prefers package, falls back to monolith."""
    try:
        # Package imports (authoritative implementation)
        import types
    ...
    try:
        ...
        print("✓ Imported gsnh_mdt_v3 (monolith fallback)")
        return gsnh
    except ImportError:
        pass
...
class DL8Benchmark:
    ...
    def _evaluate_one(self, name, X, y, n_unary):
        ...
        if 'tic-tac-toe' in name.lower():
            top_k = 27
            limit_3d_override = 27
        else:
            limit_3d_override = None    
        ...
        for run_idx, (tr_idx, te_idx) in enumerate(splitter.split(X, y)):
            ...
            g5 = ET(
                ...
                limit_3d=limit_3d_override,
                mode='journal', language=self.gsnh.LanguageFamily.BEST_PER_NODE,
            )
            g5.fit(X_tr, y_tr)
...
def main(argv=None):
    ...
    # JIT
    print("\n[2] JIT Warmup...")
    if hasattr(gsnh, 'warmup_jit'):
        gsnh.warmup_jit()
    else:
        print("  (package mode — no JIT warmup needed)")
    
    # Find data
    data_dir = resolve_data_dir(args.data_dir)
    print(f"\n[3] Resolved data directory: {data_dir}")
    try:
        discovered_files = discover_dl8_files(data_dir)
    except FileNotFoundError as exc:
        print(f"✗ {exc}")
        return None
    print(f"[3] Discovered {len(discovered_files)} .dl8 files recursively")
    
    # Load
    print(f"[3] Loading .dl8 files from {data_dir}/...")
